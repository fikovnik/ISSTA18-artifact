...
It generated ```r nrow(result)``` tests, with the code coverage of ```r max(result$coverage)```.
...
- `coverage` is the resulting coverage of R code
...
reverse dependencies. In this case we already have pretty high code coverage, but we cna try to
...
We use only direct reverse dependecies. While we can use also indirect ones, it usually does not bring much and only takes a lot of time to run.
...
increased the coverage from ```r max(result$coverage)``` to ```r max(result_rdeps$coverage)```.
