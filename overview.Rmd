...
Next, we show how the test extraction process can be automated and run on a a whole package including its reverse dependencies.
...
We can see that the predicate is captured as a symbol in the arguments and then its actual value is stored in the list of global variables. 
...
This is done using by `generate_test` method which returns a string vector with the extracted test.
...
The `generated_test_file` returns the file path. This can be fed into `test_generated_file`, which is a thin wrapper over `testthat::test_file` function taking care of the external variables.
...
Genthat is usually used in a batch processing, extracting tests for a whole package.
...
```
From the coverage of `r fmt(tests_coverage_value)`% we see that the tests does not cover much in this package.
...
- `coverage` - test coverage computed as a sum of previous rows plus this test

The reported test coverage is only for the tests generated by genthat.
Since, genthat also runs package tests (often they are far from unit tests), it should also capture all their calls.
However, in some cases genthat cannot properly trace a call and so it can be useful to include also the original tests in the final test suite.
The `compute_coverage` function can compute test coverage from multiple source.
