...
test_that("MouthSource", {
    ...
    path <- structure(c("Ballinderry", "Lissan", "Ballymully", "Kingsmill", "Killymoon-Claggan", "Rock", "Kildress", "Tulnacross", 
        ...
        "")))
    OBN <- 8L
    expect_equal(rivervis:::MouthSource(path = path, riverlayout = riverlayout, OBN = OBN), structure(c(0, 11.5, 13.2, 13.9, 
        19.7, 29, 30.5, 32.3, 45.3, 32.8, 30.3, 27.4, 48.2, 45, 39.8, 39.6), .Dim = c(8L, 2L), .Dimnames = list(c("Ballinderry", 
        "Lissan", "Ballymully", "Kingsmill", "Killymoon-Claggan", "Rock", "Kildress", "Tulnacross"), c("rmouth", "rsource"))))
})
