...
test_that("RiverSite", {
    ...
    B.town <- structure(list(Town = c("Cookstown", "Moneymore"), River = c("Ballinderry", "Ballymully"), Distance = c(22.2, 
        6.6), Group = structure(c(1L, 1L), .Label = "Town", class = "factor")), row.names = c(NA, -2L), class = "data.frame")
    expect_equal(rivervis:::RiverSite(site = B.town$Town, river = B.town$River, distance = B.town$Distance, group = B.town$Group, 
        riverlayout = riverlayout, pt.pch = 22, pt.col = "black", pt.bg = "orange", lbl.shw = FALSE), NULL)
})
