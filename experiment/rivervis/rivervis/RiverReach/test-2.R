...
test_that("RiverReach", {
    ...
    B.reach <- structure(list(Reach = "UpperBallinderry", River = "Ballinderry", From = 22.4, To = 45.28, Group = structure(1L, 
        .Label = "SAC", class = "factor"), Style = 99L), row.names = c(NA, -1L), class = "data.frame")
    expect_equal(rivervis:::RiverReach(reach = B.reach$Reach, river = B.reach$River, from = B.reach$From, to = B.reach$To, 
        group = B.reach$Group, style = 2, riverlayout = riverlayout, rea.col = "darkred", rea.lwd = 5), NULL)
})
